...
local cfgDisableMaphack = imgui.new.bool(true)
local cfgShowNametags   = imgui.new.bool(true)
local cfgDrawMethod     = imgui.new.int(2)
local cfgNametagOffsetX = imgui.new.int(0)
local cfgNametagOffsetY = imgui.new.int(0)
local cfgBarWidth       = imgui.new.int(80)
local cfgBarHeight      = imgui.new.int(8)
...
--  AUTO UPDATE CONFIG
-- ════════════════════════════════════════════════
local CURRENT_VERSION  = "5.3"   -- versi script ini (admin update via panel)
...
function syncVariablesToUi()
    cfgDisableMaphack[0] = config.settings.DisableMaphack
    cfgShowNametags[0]   = config.settings.showNametags
    cfgDrawMethod[0]     = config.settings.drawMethod
    cfgNametagOffsetX[0] = config.settings.nametagOffsetX
    cfgNametagOffsetY[0] = config.settings.nametagOffsetY
    cfgBarWidth[0]       = config.settings.barWidth
    cfgBarHeight[0]      = config.settings.barHeight
end

function syncUiToVariables()
    config.settings.DisableMaphack = cfgDisableMaphack[0]
    config.settings.showNametags   = cfgShowNametags[0]
    config.settings.drawMethod     = cfgDrawMethod[0]
    config.settings.nametagOffsetX = cfgNametagOffsetX[0]
    config.settings.nametagOffsetY = cfgNametagOffsetY[0]
    config.settings.barWidth       = cfgBarWidth[0]
    config.settings.barHeight      = cfgBarHeight[0]
end
...
function loadConfig()
    ...
    if ini then
        config = ini
        syncVariablesToUi()
        chat("{00FF00}[AutoSusu]{FFFFFF} Maphack config loaded")
    else
        chat("{FF0000}[AutoSusu]{FFFFFF} Using default maphack config")
    end
end

function saveConfig()
    syncUiToVariables()
    inicfg.save(config, "AutoSusu_Maphack")
end
...
function renderHealthAndArmourBar(draw_list, id, x, y)
    local health = sampGetPlayerHealth(id)
    if health == nil or health == 0 then return end
    
    local healthPercentage = math.min(health / 100, 1.0)
    local armor = sampGetPlayerArmor(id)
    local armorPercentage = armor and math.min(armor / 100, 1.0) or 0
    
    local width = config.settings.barWidth
    local height = config.settings.barHeight

    -- Armor Bar (Silver)
    if armorPercentage > 0 then
        draw_list:AddRectFilled(
            imgui.ImVec2(x - (width / 2 + 2), y - (height + 4)),
            imgui.ImVec2(x + (width / 2 + 2), y),
            0xFF000000
        )
        draw_list:AddRectFilled(
            imgui.ImVec2(x - width / 2, y - height - 2),
            imgui.ImVec2(x - width / 2 + (width * armorPercentage), y - 2),
            0xFFC7C5C6
        )
    end
    ...
    -- Health Bar (Blue)
    draw_list:AddRectFilled(
        imgui.ImVec2(x - (width / 2 + 2), y + 2),
        imgui.ImVec2(x + (width / 2 + 2), y + height + 6),
        0xFF000000
    )
    draw_list:AddRectFilled(
        imgui.ImVec2(x - width / 2, y + 4),
        imgui.ImVec2(x - width / 2 + (width * healthPercentage), y + height + 4),
        0xFF0000FF
    )
end
    ...
    for _, char in ipairs(getAllChars()) do
        if char ~= PLAYER_PED then
            ...
            if result and isCharOnScreen(char) and not sampIsPlayerNpc(id) then
                ...
                local distance = getDistanceBetweenCoords3d(selfX, selfY, selfZ, charX, charY, charZ)

                if distance <= maxDistance then
                    local hdx, hdy, hdz = getBonePosition(char, 5)
                    local hddr, hddx, hddy = convert3DCoordsToScreenEx(hdx, hdy, hdz)

                    if hddr and isLineOfSightClear(selfX, selfY, selfZ, charX, charY, charZ, true, false, false, true, false) then
                        local color = sampGetPlayerColor(id)
                        local r, g, b = bit.band(bit.rshift(color, 16), 0xFF), bit.band(bit.rshift(color, 8), 0xFF), bit.band(color, 0xFF)
                        local imguiColor = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(r/255, g/255, b/255, 1.0))

                        local nametag = sampGetPlayerNickname(id) .. " (" .. id .. ")"
                        local textSize = imgui.CalcTextSize(nametag)

                        local posX = hddx + config.settings.nametagOffsetX - (textSize.x / 2)
                        local posY = hddy + config.settings.nametagOffsetY - textSize.y

                        -- Render Teks (Background DrawList)
                        draw_list:AddText(imgui.ImVec2(posX, posY), imguiColor, nametag)

                        -- Render Health & Armor Bar
                        if config.settings.drawMethod == 2 then
                            renderHealthAndArmourBar(draw_list, id, hddx + config.settings.nametagOffsetX, hddy + config.settings.nametagOffsetY + 15)
                        end
                    end
                end
            end
...
--  GAME INIT EVENT
-- ════════════════════════════════════════════════
function sampEvents.onInitGame(playerId, hostName, settings, vehicleModels, unknown)
    if config.settings.DisableMaphack then
...
function main()
    ...
    chat("{00FF00}[PrimeBot v5.3]{FFFFFF} Panel Ready! Ketik {00FF00}/autosusu")
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(0,0.85,1,1), utf8("AutoSusu CPRP v5.3 — Key License"))
        imgui.Separator()
        imgui.Spacing()
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.Begin(utf8("AutoSusu CPRP v5.3 (Prime Edit)"), showPanel)
        ...
        if imgui.BeginTabBar("Tabs") then
            ...
            if imgui.BeginTabItem(utf8("🎮 Cheat Menu")) then
                ...
                -- Disable Maphack Toggle
                if imgui.Checkbox(utf8("Disable Maphack & Native Markers"), cfgDisableMaphack) then
                    saveConfig()
                    chat("{00FF00}[Maphack]{FFFFFF} Status: " .. (cfgDisableMaphack[0] and "ON" or "OFF"))
                end

                -- Show Custom Nametags Toggle
                if imgui.Checkbox(utf8("Show Custom Nametags"), cfgShowNametags) then
                    saveConfig()
                    chat("{00FF00}[Nametags]{FFFFFF} Status: " .. (cfgShowNametags[0] and "ON" or "OFF"))
                end
                ...
                -- Radio buttons untuk draw method
                if imgui.RadioButtonIntPtr(utf8("Nametag Only"), cfgDrawMethod, 1) then
                    saveConfig()
                    chat("{00FF00}[DrawMethod]{FFFFFF} Set to: Nametag Only")
                end
                
                imgui.SameLine()
                
                if imgui.RadioButtonIntPtr(utf8("Nametag + HP/Armor Bar"), cfgDrawMethod, 2) then
                    saveConfig()
                    chat("{00FF00}[DrawMethod]{FFFFFF} Set to: Nametag + Bars")
                end
                ...
                -- Sliders untuk offset dan bar size
                imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1),
                    utf8("Position & Size:"))

                if imgui.SliderInt(utf8("Offset X##maphack"), cfgNametagOffsetX, -50, 50) then
                    saveConfig()
                end

                if imgui.SliderInt(utf8("Offset Y##maphack"), cfgNametagOffsetY, -50, 50) then
                    saveConfig()
                end

                if imgui.SliderInt(utf8("Bar Width##maphack"), cfgBarWidth, 30, 150) then
                    saveConfig()
                end

                if imgui.SliderInt(utf8("Bar Height##maphack"), cfgBarHeight, 3, 20) then
                    saveConfig()
                end

                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.4, 1, 0.4, 1),
                    utf8("✓ All settings saved automatically"))
