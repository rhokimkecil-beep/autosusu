...
-- ════════════════════════════════════════════════
--  DISABLE MAPHACK STATE
-- ════════════════════════════════════════════════
local dmh_enabled    = imgui.new.bool(true)
local dmh_nametags   = imgui.new.bool(true)
local dmh_drawMethod = imgui.new.int(2)
local dmh_offsetX    = imgui.new.int(0)
local dmh_offsetY    = imgui.new.int(0)
local dmh_barWidth   = imgui.new.int(80)
local dmh_barHeight  = imgui.new.int(8)
local dmh = {enabled=true,nametags=true,drawMethod=2,offsetX=0,offsetY=0,barWidth=80,barHeight=8,maxDist=50}
...
local function getBonePos(ped, bone)
    local p = ffi.new('RwV3d[1]')
    pcall(function()
        local ptr = ffi.cast('void*', getCharPointer(ped))
        ffi.load('GTASA')._ZN4CPed15GetBonePositionER5RwV3djb(ptr, p, bone, false)
    end)
    return p[0].x, p[0].y, p[0].z
end
...
local CURRENT_VERSION  = "5.2"   -- versi script ini (admin update via panel)
...
function main()
    ...
    chat("{00FF00}[PrimeBot v5.2]{FFFFFF} Panel Ready! Ketik {00FF00}/autosusu")
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.TextColored(imgui.ImVec4(0,0.85,1,1), utf8("AutoSusu CPRP v5.2 — Key License"))
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.Begin(utf8("AutoSusu CPRP v5.2)"), showPanel)
        ...
        if imgui.BeginTabBar("Tabs") then
            ...
            if imgui.BeginTabItem(utf8("Cheat Menu")) then
                ...
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.2,0.8,1,1), utf8("Disable Maphack"))
                imgui.Separator()
                imgui.Spacing()
                if imgui.Checkbox(utf8("Aktifkan Disable Maphack"), dmh_enabled) then
                    dmh.enabled = dmh_enabled[0]
                end
                imgui.Spacing()
                if imgui.Checkbox(utf8("Tampilkan Custom Nametag"), dmh_nametags) then
                    dmh.nametags = dmh_nametags[0]
                end
                imgui.Spacing()
                imgui.Separator()
                imgui.TextColored(imgui.ImVec4(0.8,0.8,0.2,1), utf8("Tampilan Nametag:"))
                imgui.Spacing()
                if imgui.RadioButtonIntPtr(utf8("Nama Saja"), dmh_drawMethod, 1) then dmh.drawMethod = 1 end
                imgui.SameLine()
                if imgui.RadioButtonIntPtr(utf8("Nama + HP Bar"), dmh_drawMethod, 2) then dmh.drawMethod = 2 end
                imgui.Spacing()
                imgui.Separator()
                imgui.TextColored(imgui.ImVec4(0.8,0.8,0.2,1), utf8("Posisi & Ukuran:"))
                imgui.Spacing()
                if imgui.SliderInt(utf8("Offset X"), dmh_offsetX, -50, 50) then dmh.offsetX = dmh_offsetX[0] end
                if imgui.SliderInt(utf8("Offset Y"), dmh_offsetY, -50, 50) then dmh.offsetY = dmh_offsetY[0] end
                if imgui.SliderInt(utf8("Bar Width"), dmh_barWidth, 30, 150) then dmh.barWidth = dmh_barWidth[0] end
                if imgui.SliderInt(utf8("Bar Height"), dmh_barHeight, 3, 20) then dmh.barHeight = dmh_barHeight[0] end
                imgui.Spacing()
                imgui.EndTabItem()
            end
...
-- ════════════════════════════════════════════════
--  RENDER NAMETAG (Disable Maphack)
-- ════════════════════════════════════════════════
imgui.OnFrame(function() return dmh.nametags end, function()
    if not isSampLoaded() or not isSampAvailable() then return end
    local draw_list = imgui.GetBackgroundDrawList()
    local ok, playerPed = pcall(getPlayerPed, convertIntToPlayerIndex(getPlayerPed ~= nil and 0 or 0))
    if not ok then return end
    local selfX, selfY, selfZ = getCharCoordinates(PLAYER_PED)

    for _, char in ipairs(getAllChars()) do
        if char ~= PLAYER_PED then
            local res, id = sampGetPlayerIdByCharHandle(char)
            if res and isCharOnScreen(char) and not sampIsPlayerNpc(id) then
                local cx, cy, cz = getCharCoordinates(char)
                local dist = getDistanceBetweenCoords3d(selfX, selfY, selfZ, cx, cy, cz)
                if dist <= dmh.maxDist then
                    local hx, hy, hz = getBonePos(char, 5)
                    local onScreen, sx, sy = convert3DCoordsToScreenEx(hx, hy, hz)
                    if onScreen then
                        ...
                        local imcol = imgui.ColorConvertFloat4ToU32(imgui.ImVec4(r,g,b,1))
                        local name = sampGetPlayerNickname(id)..' ('..id..')'
                        local tsz  = imgui.CalcTextSize(name)
                        local tx = sx + dmh.offsetX - tsz.x/2
                        local ty = sy + dmh.offsetY - tsz.y
                        draw_list:AddText(imgui.ImVec2(tx, ty), imcol, name)
                        if dmh.drawMethod == 2 then
                            local hp  = sampGetPlayerHealth(id)/100
                            local arm = sampGetPlayerArmor(id)/100
                            local w, h = dmh.barWidth, dmh.barHeight
                            local bx = sx + dmh.offsetX
                            local by = sy + dmh.offsetY + 15
                            if arm > 0 then
                                draw_list:AddRectFilled(imgui.ImVec2(bx-(w/2+2),by-(h+4)),imgui.ImVec2(bx+(w/2+2),by),0xFF000000)
                                draw_list:AddRectFilled(imgui.ImVec2(bx-w/2,by-h-2),imgui.ImVec2(bx-w/2+(w*arm),by-2),0xFFC7C5C6)
                            end
                            draw_list:AddRectFilled(imgui.ImVec2(bx-(w/2+2),by+2),imgui.ImVec2(bx+(w/2+2),by+h+6),0xFF000000)
                            draw_list:AddRectFilled(imgui.ImVec2(bx-w/2,by+4),imgui.ImVec2(bx-w/2+(w*hp),by+h+4),0xFF0000FF)
                        end
                    end
                end
            end
